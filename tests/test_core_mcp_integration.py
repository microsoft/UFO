...
def test_core_mcp_client_basic():
    ...
    logger.info(
        f"MCP Server connection test: {'PASSED' if connection_ok else 'FAILED (Expected if server not running)'}"
    )

    if not connection_ok:
        logger.warning(
            "Core MCP Server is not running. This is expected if you haven't started it yet."
        )
        logger.info(
            "To start the Core MCP Server, run: python -m ufo.mcp.core_mcp_server"
        )
        return False
    ...
    try:
        ...
        logger.info(
            f"GetDesktopAppInfo test: PASSED (got {len(result) if isinstance(result, list) else 'non-list'} apps)"
        )
        return True
...
def test_ufo_web_client_with_mcp():
    ...
    logger.info(
        f"UFOWebClient initialized with use_core_mcp_server={client.use_core_mcp_server}"
    )
...
def test_ufo_web_client_without_mcp():
    ...
    client = UFOWebClient(
        server_url="http://localhost:5000", computer_name="test_client"
    )
...
def main():
    ...
    logger.info(
        f"UFOWebClient without MCP Test: {'PASSED' if test3_passed else 'FAILED'}"
    )
    ...
    if test2_passed and test3_passed:
        ...
        logger.info(
            "📝 To start the Core MCP Server: python -m ufo.mcp.core_mcp_server"
        )
        return True
    else:
