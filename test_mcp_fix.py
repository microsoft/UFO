    ...
    try:
        ...
        try:
            ...
            if result.get("available"):
                ...
                print(
                    f"✓ Successfully retrieved {len(tools)} tools from core MCP server"
                )
                print(
                    f"   Sample tools: {[tool.get('name', 'Unknown') for tool in tools[:3]]}"
                )
            else:
        ...
        try:
            ...
            if result.get("available"):
                ...
                print(
                    f"✓ Successfully retrieved {len(tools)} tools from powerpoint (source: {source})"
                )
                if tools:
                    print(
                        f"   Sample tools: {[tool.get('name', 'Unknown') for tool in tools[:3]]}"
...
def main():
    ...
    if success:
        ...
        print(
            "The issue has been resolved - no more HTTP requests to non-existent endpoints."
        )
    else:
