...
class Computer:
    ...
    def _init_mcp_servers(self):
            ...
            if os.path.exists(servers_config_path):
                    ...
                    if server_config.get("enabled", True):
                        mcp_configs[namespace] = {
                            ...
                            "instructions_path": os.path.join(
                                base_dir, server_config["instructions_path"]
                            ),
                        }
            else:
                ...
                mcp_configs = {
                    "powerpoint": {
                        ...
                        "instructions_path": os.path.join(
                            base_dir, "config", "mcp_instructions", "powerpoint.yaml"
                        ),
                    },
                    "word": {
                        "endpoint": "http://localhost:8002",
                        "instructions_path": os.path.join(
                            base_dir, "config", "mcp_instructions", "word.yaml"
                        ),
                    },
                    "excel": {
                        "endpoint": "http://localhost:8003",
                        "instructions_path": os.path.join(
                            base_dir, "config", "mcp_instructions", "excel.yaml"
                        ),
                    },
                    "web": {
                        "endpoint": "http://localhost:8004",
                        "instructions_path": os.path.join(
                            base_dir, "config", "mcp_instructions", "web.yaml"
                        ),
                    },
                    "shell": {
                        "endpoint": "http://localhost:8005",
                        "instructions_path": os.path.join(
                            base_dir, "config", "mcp_instructions", "shell.yaml"
                        ),
                    },
                    "hardware": {
                        "endpoint": "http://localhost:8006",
                        "instructions_path": os.path.join(
                            base_dir, "config", "mcp_instructions", "hardware.yaml"
                        ),
                    },
                }
        ...
        for namespace, config in mcp_configs.items():
            try:
                ...
                print_with_color(
                    f"Failed to initialize MCP server {namespace}: {e}", "red"
                )
    ...
    def run_action(self, action: ActionBase) -> Dict[str, Any]:
        ...
        action_handlers = {
            ...
            "capture_app_window_screenshot_from_webcam": self._handle_capture_app_window_screenshot_from_webcam,
    ...
    def _handle_operation_sequence(self, action: ActionBase) -> Dict[str, Any]:
        ...
        application_window = self._get_window_by_annotation_id(
            self.selected_app_window_info.annotation_id
        )
        for command in action_commands:
            ...
            args = {
                key: value
                for key, value in command_dict[command_id].items()
                if key not in ["control_label", "control_text", "after_status"]
            }
    ...
    def _handle_capture_desktop_screenshot(
    ...
    def _handle_capture_app_window_screenshot(
        ...
        if not app_window:
            raise ValueError(
                "Window handle is required for capture_app_window_screenshot"
            )

        # Capture screenshot to a temporary location
        self.photographer.capture_app_window_screenshot(app_window, save_path=temp_path)
    ...
    def _handle_capture_app_window_screenshot_from_webcam(
    ...
    def _handle_get_app_window_control_info(
        ...
        self.photographer.capture_app_window_screenshot(
            application_window, save_path=temp_path
        )
        ...
        return AppWindowControlInfo(window_info=window_info, controls=control_elements)
    ...
    def _handle_find_control_elements(
        ...
        if not control_type_list and not class_name_list:
            raise ValueError(
                "At least one of control_type_list or class_name_list is required for find_control_elements"
            )
        control_elements = self.control_inspector.find_control_elements_in_descendants(
            window, control_type_list=control_type_list, class_name_list=class_name_list
        )

        return {"control_elements": control_elements}

    def _handle_select_application_window(
    ...
    def _handle_launch_application(
        self, action: LaunchApplicationAction
    ) -> Dict[str, Any]:
        ...
        for win_info in self.last_app_windows_info:
            if (
                hasattr(window, "process_id")
                and window.process_id() == win_info.process_id
            ):
        ...
        if not window_found:
            self.selected_app_window_info = WindowInfo(
                ...
                name=(
                    getattr(window.element_info, "name", None)
                ...
                class_name=(
                    window.class_name() if hasattr(window, "class_name") else None
                ),
                process_id=(
                    window.process_id() if hasattr(window, "process_id") else None
                ),
                is_visible=(
                    window.is_visible() if hasattr(window, "is_visible") else False
                ),
                is_minimized=(
                    window.is_minimized() if hasattr(window, "is_minimized") else False
                ),
                is_maximized=(
                    window.is_maximized() if hasattr(window, "is_maximized") else False
                ),
                rectangle=self._get_control_rectangle(window),
                text_content=(
                    window.window_text() if hasattr(window, "window_text") else None
                ),
            )
        ...
        return {
            "process_name": process_name,
            "window_text": window_text,
            "desk_top_windows_info": [
                w.model_dump() for w in self.last_app_windows_info
            ],
            "window_info": self.selected_app_window_info.model_dump(),
        }
    ...
    def _handle_get_mcp_instructions(
    ...
    def _handle_get_mcp_available_tools(
        ...
        app_namespace = params.app_namespace
        if app_namespace not in self.mcp_servers:
            return {"namespace": app_namespace, "tools": [], "available": False}
        ...
        if source == "mcp":
            ...
            for tool in tools_data:
                ...
                normalized_tool = {
                    ...
                    "parameters": self._convert_mcp_parameters_to_yaml(
    ...
    def _convert_mcp_parameters_to_yaml(
...
if __name__ == "__main__":
    ...
    app_windows = computer.run_action(
        GetDesktopAppInfoAction(params=GetDesktopAppInfoParams(remove_empty=True))
    )
