...
def test_office_app_session():
    ...
    session_id = str(uuid4())
    session = ServiceSession(task=session_id, should_evaluate=False, id=session_id)
    session.init(request="open PowerPoint and create a new slide with title 'Hello World'")
    ...
    while not is_finished:
        ...
        for action in actions:
            ...
            if hasattr(result, '__class__') and hasattr(result.__class__, '__name__') and hasattr(result, 'model_dump'):
                try:
