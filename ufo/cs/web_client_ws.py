...
class UFOWebClient:
    ...
    def __init__(
        ...
        if self.use_core_mcp_server:
            ...
            logger.info(
                f"Core MCP client initialized for {core_mcp_host}:{core_mcp_port}"
            )
            # Test MCP connection
            if not self.mcp_client.test_connection():
                logger.warning(
                    "Failed to connect to Core MCP server, falling back to direct computer execution"
                )
    ...
    async with websockets.connect(server_addr) as ws:
        ...
        async for msg in ws:
            ...
            if task.get("type") == "task":
                ...
                ufo_web_client.session_id = None  # 每次新任务重置session_id
                # reuse the request_text
                success = ufo_web_client.run_task(request_text)
