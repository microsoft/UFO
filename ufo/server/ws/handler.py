...
class UFOWebSocketHandler:
    ...
    async def handle_message(self, msg: str, websocket: WebSocket):
        ...
        try:
            ...
            if msg_type == "task":
                ...
                await self.handle_command_results(data, websocket, client_id)
            elif msg_type == "heartbeat":
                await self.handle_heartbeat(websocket, client_id)
            elif msg_type == "error":
                await self.handle_error(data, websocket, client_id)
            else:
    ...
    async def handle_task_request(self, data: ClientMessage, websocket: WebSocket):
        ...
        :param websocket: The WebSocket connection.
        """
        self.logger.info(f"[WS] Handling task request: {data}")
        ...
        try:
            await session.run()
            status = "completed"
            self.logger.info(f"[WS] Task {session_id} completed successfully")
    ...
    async def handle_command_result(self, data: ClientMessage):
        ...
        message_bus = session.message_bus
