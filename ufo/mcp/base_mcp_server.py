...
class BaseMCPServer:
    ...
    def _load_instructions(self) -> Dict[str, Any]:
        ...
        try:
            instructions_path = os.path.join(
                ...
                "config",
                "mcp_instructions",
                f"{self.app_namespace}.yaml",
            )
            if os.path.exists(instructions_path):
                with open(instructions_path, "r") as f:
                    instructions = yaml.safe_load(f)
                print_with_color(
                    f"Loaded instructions for {self.app_namespace}", "green"
                )
                return instructions
            else:
                print_with_color(
                    f"Instructions file not found: {instructions_path}", "yellow"
                )
                return {"tools": []}
    ...
    def _create_receiver(self):
        ...
        try:
            ...
            # For shell, hardware, no app_root_name or process_name needed

            receiver = factory.create_receiver(
                self.app_namespace, app_root_name, process_name
            )
            if receiver:
                ...
                print_with_color(
                    f"Failed to create {self.app_namespace} receiver", "red"
                )
                return None
        except Exception as e:
    ...
    def _register_tools(self):
        ...
        if not self.instructions or "tools" not in self.instructions:
            print_with_color(
                f"No tools found in instructions for {self.app_namespace}", "yellow"
            )
            return
    ...
    def _register_tool(self, tool_def: Dict[str, Any]):
        ...
        print_with_color(
            f"Attempting to register {tool_name} with {len(parameters)} parameters",
            "cyan",
        )
        ...
        try:
            ...
            print_with_color(
                f"✓ Successfully registered {tool_name} with parameters", "green"
            )
        except Exception as e:
            print_with_color(
                f"✗ Parameter registration failed for {tool_name}: {e}", "red"
            )
            print_with_color(
                f"  Falling back to simple registration for {tool_name}", "yellow"
            )
            self._register_simple_tool(tool_name, description)

    def _register_tool_with_parameters(
        self, tool_name: str, description: str, parameters: list
    ):
        """Register tool with proper FastMCP parameter handling."""
        print_with_color(
            f"  🔧 _register_tool_with_parameters called for {tool_name}", "blue"
        )
        ...
        try:
            ...
            if not parameters:
                print_with_color(
                    f"  📝 {tool_name} has no parameters, creating simple function",
                    "blue",
                )
                ...
                print_with_color(
                    f"Registered tool: {tool_name} with no parameters", "green"
                )
                return

            print_with_color(
                f"  📋 Processing {len(parameters)} parameters for {tool_name}: {[p.get('name') for p in parameters]}",
                "blue",
            )
            ...
            for i, param in enumerate(parameters):
                ...
                print_with_color(
                    f"    - Parameter {i+1}: {param_name} (type: {param_type}, required: {param_required}, default: {param_default})",
                    "cyan",
                )
                ...
                print_with_color(
                    f"      → Mapped to Python type: {python_type.__name__}", "cyan"
                )
                ...
                if param_required and param_default is None:
                    ...
                    print_with_color(
                        f"      → Created required parameter: {param_name}", "cyan"
                    )
                else:
                    ...
                    print_with_color(
                        f"      → Created optional parameter: {param_name} = {default_val}",
                        "cyan",
                    )
                    ...
                    try:
                        ...
                        for param in parameters:
                            ...
                            if param_required and param_default is None:
                                ...
                                param_name not in processed_args
            ...
            try:
                ...
                print_with_color(
                    f"  ✅ Function registered with FastMCP successfully", "blue"
                )
            except Exception as e:
            ...
            print_with_color(
                f"Registered tool: {tool_name} with {len(parameters)} parameters: {param_names}",
                "green",
            )
    ...
    def _execute_tool(
        ...
        try:
            ...
            if not hasattr(self.receiver, "execute_tool"):
                ...
                if hasattr(self.receiver, "execute"):
                    ...
                    return {
                        ...
                        "tool_name": tool_name,
                        "available_methods": [
                            m for m in dir(self.receiver) if not m.startswith("_")
                        ],
                    }
            else:
            ...
            if not isinstance(result, dict):
                result = {"success": True, "result": result, "tool_name": tool_name}

            print_with_color(
                f"Tool {tool_name} executed successfully: {result.get('success', 'unknown')}",
                "green",
            )
            return result
            ...
            print_with_color(
                f"AttributeError executing {tool_name}: {error_msg}", "red"
            )
            return {
                "success": False,
                "error": error_msg,
    ...
    def run(self):
        ...
        """
        print_with_color(
            f"Starting {self.app_namespace} MCP server: {self.server_name}", "blue"
        )

        if self.instructions:
            print_with_color(
                f"Loaded {len(self.instructions.get('tools', []))} tools", "green"
            )
    ...
    def get_tools(self):
        ...
        if hasattr(self.mcp, "_tools"):
            ...
            if self.instructions and "tools" in self.instructions:
                return [
                    tool.get("name")
                    for tool in self.instructions["tools"]
                    if tool.get("name")
                ]
            return []
    ...
    def _register_diagnostic_tools(self):
        ...
        def diagnose_server() -> Dict[str, Any]:
            ...
            return {
                ...
                "receiver_type": (
                    type(self.receiver).__name__ if self.receiver else None
                ),
        ...
        print_with_color(
            "Registered diagnostic tools: diagnose_server, test_receiver", "blue"
        )

    def _print_startup_diagnostics(self):
        """Print diagnostic information at startup."""
        print_with_color(
            f"=== {self.app_namespace.upper()} MCP Server Diagnostics ===", "blue"
        )
        print_with_color(f"Server Name: {self.server_name}", "white")
        print_with_color(f"Port: {self.port if self.port else 'stdio'}", "white")
        print_with_color(
            f"Instructions Loaded: {'✓' if self.instructions else '✗'}",
            "green" if self.instructions else "red",
        )
        print_with_color(
            f"Receiver Created: {'✓' if self.receiver else '✗'}",
            "green" if self.receiver else "red",
        )

        if self.receiver:
            print_with_color(f"Receiver Type: {type(self.receiver).__name__}", "white")
            print_with_color(
                f"Has execute_tool: {'✓' if hasattr(self.receiver, 'execute_tool') else '✗'}",
...
def create_mcp_server(
    app_namespace: str, server_name: str = None, port: int = None
) -> BaseMCPServer:
    """
...
if __name__ == "__main__":
    ...
    print(
        "Run specific application servers like word_mcp_server.py, excel_mcp_server.py, etc."
    )
