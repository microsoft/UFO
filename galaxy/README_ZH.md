...
自然语言或编程请求由 **ConstellationAgent（星座智能体）**分解为结构化的 DAG，包含编码工作流逻辑、依赖关系和设备分配的 **TaskStars（任务星）**（节点）和 **TaskStarLines（任务星线）**（边）。
...
**TaskConstellation（任务星座）**是一个响应执行反馈而演化的活动数据结构。中间输出、瞬态故障和新观察触发受控的重写。

**适应机制：**
- 🩺 添加**诊断 TaskStars** 用于调试
- 🛡️ **创建回退方案**用于错误恢复
- 🔗 **依赖关系重连**用于工作流优化
- ✂️ 完成后**节点修剪**

这使得系统能够动态适应而不是在错误时中止，实现真正的弹性。

---

### ⚡ 异构、异步与安全编排

每个 **TaskStar** 通过反映操作系统、硬件能力和已安装工具的丰富 **AgentProfiles（智能体配置文件）**匹配到最合适的设备智能体。**Constellation Orchestrator（星座编排器）**异步执行任务，允许多个 TaskStars 并行进行。
...
基于持久的 **WebSocket 通道**构建，AIP 为整个智能体生态系统提供统一、安全和容错的层。
