...
class ChooseTemplateFlow:
    ...
    def _create_copied_file(self, copy_from_path: Path, copy_to_folder_path: Path, file_name: str = None) -> str:
        """
        ...
        copied_template_path = self._generate_copied_file_path(copy_to_folder_path, file_name)
    ...
    def _get_chosen_file_path(self) -> str:
        ...
        """
        templates_description_path = Path(configs["TEMPLATE_PATH"]) /\
              self._app_object.description.lower() / "description.json"

        try:
            with open(templates_description_path, "r") as f:
                return self._choose_target_template_file(self._task_file_name, json.load(f))
        except FileNotFoundError:
            warnings.warn(f"Warning: {templates_description_path} does not exist. Choosing a random template.")
    ...
    def _choose_random_template(self) -> str:
        ...
        """
        template_folder = Path(configs["TEMPLATE_PATH"]) / self._app_object.description.lower()
    ...
    def _choose_template_and_copy(self) -> str:
        ...
        chosen_template_full_path = Path(configs["TEMPLATE_PATH"]) / \
            self._app_object.description.lower() / chosen_template_file_path

        target_template_folder_path = Path(configs["TASKS_HUB"]) / \
            (os.path.dirname(os.path.dirname(self._task_file_name)) + "_templates")

        return self._create_copied_file(chosen_template_full_path, target_template_folder_path, self._task_file_name)

    def _choose_target_template_file(self, given_task: str, doc_files_description: Dict[str, str]) -> str:
        """
        ...
        """
        file_doc_map = {desc: file_name for file_name, desc in doc_files_description.items()}
        db = FAISS.from_texts(list(doc_files_description.values()), self._embedding_model)
    ...
    def _load_embedding_model(model_name: str) -> CacheBackedEmbeddings:
        ...
        return CacheBackedEmbeddings.from_bytes_store(embedding_model, store, namespace=model_name)
